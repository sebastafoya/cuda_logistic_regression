...
static void LogisticRegression(Matrix *X, Matrix *y, Matrix *Parameters, Matrix *Train_Parameters, int maxIterations, float alpha, vector<float> &cost_function)
{
	// put data into gpu
	float *gpu_X;
	float *gpu_y;
...
int main(int argc, char *argv[])
{
    ...
    LogisticRegression(&X, &y, &Parameters, &Train_Parameters, 150, 0.03, cost_function);
    //cutStopTimer(timer);
